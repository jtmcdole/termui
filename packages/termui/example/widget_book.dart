...
void main(List<String> arguments) async {
  // 1. Setup argument parsing for rendering mode selection
  ...
  // 2. Run the application inside the crash protection zone
  await term.Terminal.runGuarded((terminal) async {
    ...
    var lastFpsMs = DateTime.now().millisecondsSinceEpoch;
    var fpsFrameCount = 0;
    var currentFps = 0.0;

    // Timer to drive animations at ~60fps
    final animationTimer = Timer.periodic(const Duration(milliseconds: 16), (
      timer,
    ) {
      for (final example in examples.values) {
