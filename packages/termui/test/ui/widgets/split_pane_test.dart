  ...
  group('SplitPane Tests', () {
    test('SplitPane handles resizing via mouse drag', () {
      final pane = SplitPane(
        ...
        constraint1: const PercentageConstraint(50), constraint2: const PercentageConstraint(50),
      ...
      (element as dynamic).handleMouseEvent(
        evt.MouseEvent(x: 10, y: 5, button: evt.MouseButton.left, type: evt.MouseEventType.press, modifiers: const {}), 10, 5
      );

      (element as dynamic).handleMouseEvent(
        evt.MouseEvent(x: 15, y: 5, button: evt.MouseButton.left, type: evt.MouseEventType.drag, modifiers: const {}), 15, 5
      );

      (element as dynamic).handleMouseEvent(
        evt.MouseEvent(x: 15, y: 5, button: evt.MouseButton.left, type: evt.MouseEventType.release, modifiers: const {}), 15, 5
      );
    ...
    test('SplitPane axis vertical mouse drag', () {
      ...
      // splitter at y=10
      (element as dynamic).handleMouseEvent(
        evt.MouseEvent(x: 5, y: 10, button: evt.MouseButton.left, type: evt.MouseEventType.press, modifiers: const {}), 5, 10
      );

      (element as dynamic).handleMouseEvent(
        evt.MouseEvent(x: 5, y: 15, button: evt.MouseButton.left, type: evt.MouseEventType.drag, modifiers: const {}), 5, 15
      );
    });

    test('SplitPane renders correctly', () async {
      final tester = TerminalTester();
      await tester.pumpWidget(SplitPane(
          child1: const Text('A'),
          child2: const Text('B'),
          direction: LayoutDirection.horizontal,
