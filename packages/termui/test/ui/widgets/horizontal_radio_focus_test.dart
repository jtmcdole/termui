...
void main() {
  group('HorizontalRadioGroup Tests', () {
    test('Selects option via keyboard', () {
      ...
      final group = HorizontalRadioGroup(controller: controller, focused: true);
      ...
      (element as dynamic).state.handleKeyEvent(
        const evt.KeyEvent('right', evt.KeyType.character),
      );
      expect(controller.focusedIndex, 1);

      (element as dynamic).state.handleKeyEvent(
        const evt.KeyEvent('left', evt.KeyType.character),
      );
  ...
  group('FocusScopeNode Tests', () {
    test('FocusScopeNode logic', () {
      final focus = Focus(child: const Text('Foo'), onFocusChange: (f) {});
